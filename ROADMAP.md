...
- **No caching of fetched transcripts across runs.** The same earnings call transcript can get fetched fresh by both the pre-earnings and post-earnings pipelines for the same ticker within days of each other. A simple content-addressed cache (keyed by ticker + report date) would cut redundant API spend and latency without changing any synthesis logic.
