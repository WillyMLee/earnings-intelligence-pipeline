    ...
    <div className="mx-auto max-w-[1500px] px-5 py-10 sm:px-8 sm:py-14">
      <header className="mb-7 flex flex-wrap items-end justify-between gap-5">
        <div>
          ...
          <h1 className="mt-2 text-[32px] font-extrabold tracking-tight text-[#15171c] dark:text-[#e7e8ea] sm:text-[40px]">Earnings Calendar</h1>
        ...
        <div className="flex rounded-xl border border-black/[0.07] bg-white p-1 dark:border-white/[0.09] dark:bg-[#121317]">
          {(Object.keys(SEASONS) as Season[]).map((key) => (
            <button key={key} onClick={() => setSeason(key)} className={`rounded-lg px-3.5 py-2 text-left transition-colors ${season === key ? "bg-accent-soft text-accent dark:bg-accent/15" : "text-[#6f7480] dark:text-[#9a9ea8]"}`}>
      ...
      <div className="overflow-x-auto rounded-card border border-black/[0.06] bg-white dark:border-white/[0.08] dark:bg-[#121317]">
            ...
            {view.rows.map((event, index) => {
              ...
              return (
                <tr key={`${event.ticker}:${event.reportDate}`} className={index ? "border-t border-black/[0.05] dark:border-white/[0.06]" : ""}>
                  <td className="whitespace-nowrap px-4 py-3"><div className="text-[12px] font-semibold text-[#2b2e35] dark:text-[#dcdee2]">{fmtDate(event.reportDate)}</div><div className="mt-0.5 text-[10px] text-[#9a9ea8]">{event.reportTime}</div></td>
