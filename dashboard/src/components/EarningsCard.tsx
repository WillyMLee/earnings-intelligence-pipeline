...
export function EarningsCard({ summary }: { summary: PostEarningsSummary }) {
    ...
    <article className="rounded-card border border-black/[0.06] bg-white p-5 shadow-[0_1px_2px_rgba(15,15,20,0.04)] dark:border-white/[0.08] dark:bg-[#121317]">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-black/[0.04] font-mono text-[13px] font-bold text-[#15171c] dark:bg-white/[0.06] dark:text-[#e7e8ea]">
            {summary.ticker.slice(0, 4)}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-[15px] font-semibold text-[#15171c] dark:text-[#e7e8ea]">{summary.company}</h3>
